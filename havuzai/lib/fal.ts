...
const FAL_MODEL = "fal-ai/nano-banana/edit"; // $0.039/görsel
...
// Bahçe fotoğrafına magenta yerleşim kılavuzu çizer: solid dikdörtgen = havuz
// ayak izi, ince dış dikdörtgen = döşeme dış sınırı, kesikli çizgi = uzun eksen.
// Kutu, bahçe fotoğrafının yatay/dikey oluşuna göre otomatik yönlenir ve
// havuzun gerçek en-boy oranına (size'dan) göre şekillenir.
async function createPlacementGuide(
  ...
  if (isLandscape) {
    ...
    guideHeight = poolAspect ? Math.round(guideWidth / poolAspect) : Math.round(height * 0.13);
  } else {
    guideHeight = Math.round(height * 0.36);
    guideWidth = poolAspect ? Math.round(guideHeight / poolAspect) : Math.round(width * 0.18);
  }
...
// İki havuz referansını tek yan-yana görsele birleştirir — ucuz model çok
// referansta dağılıyor, tek güçlü referansta şekli daha iyi kopyalıyor.
...
// Çıktıda magenta kalıntısı var mı diye örnekleyerek kontrol eder.
...
export async function generatePoolVisualization(
  ...
  // Bahçe fotoğrafına her zaman kılavuz çizilir (yön otomatik, boyuta göre).
  ...
  // Havuz referansları tek board'da birleştirilir → Image 2 her zaman tek.
  const poolReference = await createPoolReferenceBoard(poolRef, model?.reference_image_url_2);

  // Seçilen seramik/deck renginin referans görseli varsa eklenir.
  const ceramicSel = config.ceramic ? clientConfig.ceramic_colors.find((c) => c.id === config.ceramic) : null;
  const deckSel = config.deck ? clientConfig.deck_colors.find((d) => d.id === config.deck) : null;
  const materialRef =
  ...
  console.log("Kılavuz çizildi: true");
