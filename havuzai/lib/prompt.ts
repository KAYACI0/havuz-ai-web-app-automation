...
${shapeLine}
Size ${size} meters — keep the proportions. The pool stays clearly smaller than the house. Leave a generous, clearly visible strip of lawn on every side beyond the paving/deck — the finished pool area (water + paving) must never reach the photo's left or right edge, and must never fill more than roughly two-thirds of the visible garden's width. If the guide box appears to leave little margin, still keep an untouched grass border outside the paving rather than extending it further.
