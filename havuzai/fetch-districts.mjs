...
const PROVINCES = [
  ...
  { id: 37, name: "Kastamonu" }, { id: 38, name: "Kayseri" }, { id: 71, name: "Kırıkkale" },
  { id: 39, name: "Kırklareli" }, { id: 40, name: "Kırşehir" }, { id: 79, name: "Kilis" },
  ...
  { id: 58, name: "Sivas" }, { id: 63, name: "Şanlıurfa" }, { id: 73, name: "Şırnak" },
...
async function main() {
  ...
  for (const p of PROVINCES) {
    const url = `https://api.turkiyeapi.dev/v2/districts?provinceId=${p.id}&fields=id,name&sort=name&limit=100`;
    const res = await fetch(url);
    if (!res.ok) { console.error(`HATA: ${p.name} (${p.id}) -> ${res.status}`); continue; }
    const data = await res.json();
    result[p.id] = (data.data || []).map((d) => ({ id: d.id, name: d.name }));
    console.log(`${p.name}: ${result[p.id].length} ilce`);
    await sleep(150);
  }
  writeFileSync("./districts.json", JSON.stringify(result, null, 2), "utf-8");
  console.log("\nBitti -> districts.json yazildi.");
}
